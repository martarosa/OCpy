...
class ReadNamelistOC():
    ...
    def set_n_sections(self, n_sections):
        self.n_sections = n_sections

    def read_file(self, folder, namefile):
        user_input = configparser.ConfigParser()
        user_input.read_dict({'SYSTEM': {'folder': folder,
                                         'name': 'output',
                                         'nstep': '10000',
                                         'dt': '0.01',
                                         'propagation': 'eulero_2order'},
                              'WAVEFUNCTION': {'name_ci': 'ci_ini.inp',
        ...
        self.set_n_sections(len(user_input.sections()))
    ...
    def modify_oc_restart_par(self, new_val):
        self.oc.par["restart"] = new_val
        print("RESTART FILE NOT FOUND, STARTING CALCULATION FROM SCRATCH")
