...
class ClassicalPropagatorTerms(ABCPropagatorTerms):
    ...
    def eulero1_coeff_term(self, order, dt, mol, *args):
        mol.wf.ci_prev[0] = np.copy(mol.wf.ci)
        mol.wf.ci = np.copy(mol.wf.ci_prev[0])

    def eulero2_coeff_term(self, order, dt, mol, *args):
        mol.wf.ci_prev[1] = np.copy(mol.wf.ci_prev[0])
        mol.wf.ci_prev[0] = np.copy(mol.wf.ci)
        mol.wf.ci = np.copy(mol.wf.ci_prev[1])

    def eulero_energy_term(self, order, dt, mol, *args):
        mol.wf.ci += -order * 1j * dt * (mol.par.en_ci * mol.wf.ci_prev[0])

    def eulero_field_term(self, order, dt, mol, field_dt_vector, *args):
        mol.wf.ci += -order * 1j * dt * (-np.dot(np.dot(mol.wf.ci_prev[0], mol.par.muT), field_dt_vector))


    def eulero_medium_term(self, order, dt, mol, field_dt_vector, medium, *args):
        if medium != "vac":
            medium.propagate(mol, field_dt_vector)
            mol.wf.ci += -order * 1j * dt \
                          * (np.dot(mol.wf.ci_prev[0],
                                    af.single_summation_tessere(medium.get_q_t() - medium.q00n, mol.par.Vijn)))


    def bwd_medium_term(self, order, dt, mol, field_dt_vector, medium, wf_fwd, *args):
        medium.propagate_bwd_oc(wf_fwd, field_dt_vector)
        mol.wf.ci += -order * 1j * dt \
                          * (np.dot(mol.wf.ci_prev[0],
                                    af.single_summation_tessere(medium.get_q_t() - medium.q00n,
                                                                mol.par.Vijn))
                             + np.dot(wf_fwd,
    ...
    def eulero_medium_term_fortran(self, order, dt, mol, field_dt_vector, medium, *args):
        if medium.par.medium != "vac":
            medium.propagate_fortran(mol, field_dt_vector)
            q_t = medium.get_q_t()
            mol.wf.ci += -order * 1j * dt \
    ...
    def bwd_medium_term_fortran(self, order, dt, mol, field_dt_vector, medium, wf_fwd, *args):
        medium.propagate_bwd_oc_fortran(wf_fwd, field_dt_vector)
        q_t = medium.get_q_t() - medium.q00n
        mol.wf.ci += -order * 1j * dt \
