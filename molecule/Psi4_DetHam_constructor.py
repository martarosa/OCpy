...
"""
Modified by Davide on 10-05-2020
"""
...
class Determinant:
    ...
    def get_number_of_dets(nmo, nvir, nocc):
        #### sum_j^{max_excitation} (nocc over j) * (nvir over j)
        pass
...
class HamiltonianGenerator:
    ...
    def update_hamiltonian_at_time_t(self, detList, prev_hamiltonian):
        """
        Update Hamiltonian representation only for the elements differing by one orbital excitation (i.e. where one electron integrals are relevant)
        """
        numDet = len(detList)
        matrix = np.zeros((numDet, numDet))
        for i in range(numDet):
            for j in range(i + 1):
                if detList[i].numberOfTotalDiffOrbitals(detList[j]) < 2:
                    matrix[i, j] = self.calcMatrixElement(detList[i], detList[j])
                else:
                    matrix[i, j] = prev_hamiltonian[i, j]
                matrix[j, i] = matrix[i, j]
        return matrix
    


class Psi4InterfaceMolecule:

#    def __init__(self):
#        pass

    @staticmethod
    def set_n_electrons_basis_set(molecule_parameters):
        occList = [i for i in range(molecule_parameters.doubly_occupied_mo)]
        det_ref = Determinant(alphaObtList=occList, betaObtList=occList)
        detList = det_ref.generateSingleAndDoubleExcitationsOfDet(molecule_parameters.num_mo)  
        detList.append(det_ref)
        return detList
    
    
    @staticmethod
    def molecular_hamiltonian_configuration_basis(molecule_parameters):
        Hamiltonian = Psi4InterfaceMolecule.compute_CISD_hamiltonian(molecule_parameters)
        return Hamiltonian
        
    @staticmethod    
    def compute_CISD_hamiltonian(molecule_parameters):
        single_particle_hamiltonian = Psi4InterfaceMolecule.compute_single_particle_hamiltonian(molecule_parameters)
        hamiltonian_generator= HamiltonianGenerator(single_particle_hamiltonian, molecule_parameters.two_electron_int)
        Hamiltonian_matrix = hamiltonian_generator.generateMatrix(molecule_parameters.basis_determinants)
        return Hamiltonian_matrix
    
    @staticmethod
    def electron_nuclei_determinant_basis(molecule_parameters):
        H = np.einsum('uj,vi,uv', molecule_parameters.ao_coefficients, molecule_parameters.ao_coefficients, molecule_parameters.ao_potential_integral)
        H = np.repeat(H, 2, axis=0)
        H = np.repeat(H, 2, axis=1)
        spin_ind = np.arange(H.shape[0], dtype=np.int) % 2
        H *= (spin_ind.reshape(-1, 1) == spin_ind)
        hamiltonian_generator = HamiltonianGenerator(H, np.zeros(np.shape(molecule_parameters.two_electron_int)))
        H = hamiltonian_generator.generateMatrix(molecule_parameters.basis_determinants)
        return H
    
    @staticmethod    
    def compute_single_particle_hamiltonian(molecule_parameters):
        """ 
        Short explanation of the function: electron integrals are computed by psi4 mints library in the AO basis
        and take into account only the SPATIAL part, thus the second line of the function transforms the one-el integral matrix
        into the MO basis via self.par.ao_coefficients which is the basis change matrix AO->MO.
        Third and fourth line expand the matrix by simply replicating it (matrix in the tensor product space cartesian \otimes spin) 
        then the matrix elements with orthogonal spin are canceled out.
        """
        H = molecule_parameters.ao_kinetic_integral + molecule_parameters.ao_potential_integral
        H = np.einsum('uj,vi,uv', molecule_parameters.ao_coefficients, molecule_parameters.ao_coefficients, H)
        H = np.repeat(H, 2, axis=0)
        H = np.repeat(H, 2, axis=1)
        spin_ind = np.arange(H.shape[0], dtype=np.int) % 2
        H *= (spin_ind.reshape(-1, 1) == spin_ind)
